...
- If sequential mode is **false**: Read `{{KIRO_DIR}}/settings/rules/tasks-parallel-analysis.md` for parallel criteria
...
- Apply `(P)` markers for parallelizable tasks (omit markers when sequential mode is requested)
