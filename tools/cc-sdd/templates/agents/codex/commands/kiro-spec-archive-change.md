...
<instructions>
## Core Task
Archive completed change for feature **$1** and update original specifications.
...
- Read `{{KIRO_DIR}}/specs/$1/change-tasks.md` and verify all checkboxes are `[x]`

**If preconditions fail**:
- If not all tasks complete: "Complete all change tasks first with `/prompts:kiro-spec-change-impl $1`"
...
- Archive directory: `{{KIRO_DIR}}/specs/$1/archives/change-NNN/`

### Step 3: Create Archive

1. Create archive directory: `{{KIRO_DIR}}/specs/$1/archives/change-NNN/`
...
### Language Reminder
- Markdown prompt content must remain in English, even when spec.json requests another language for output. The generated output files should use the spec language.
</instructions>
...
1. **Archive Status**: Confirm archive created at `{{KIRO_DIR}}/specs/$1/archives/change-NNN/`
2. **Updated Files**:
...
- **Suggested Action**: "Run `/prompts:kiro-spec-change-impl $1` to complete remaining tasks"

**No Active Change**:
- **Stop Execution**: No change to archive
- **Message**: "No active change found for `$1`"
...
- **Message**: "change-design.md is missing integrated requirements or design sections. Re-run `/prompts:kiro-spec-change-design $1`"

### Post-Archive
- Feature specifications are updated and ready for:
  - New change requests: `/prompts:kiro-spec-change $1 "new change"`
  - Status check: `/prompts:kiro-spec-status $1`
