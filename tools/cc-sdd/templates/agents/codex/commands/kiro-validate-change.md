...
4. **Provide Decision**:
   ...
   - Persist the validation/review result as a Markdown file before completing the workflow
...
### Language Reminder
- Markdown prompt content must remain in English, even when spec.json requests another language for output. The validation report should use the spec language.
</instructions>
...
5. **Persistence**: Validation/review result is written and kept as a Markdown file
...
### Error Scenarios
- **Missing Change Request**: If change-request.md doesn't exist, stop: "Run `/prompts:kiro-spec-change $1` first"
...
### Next Phase
**If GO**: Proceed to `/prompts:kiro-spec-change-design $1`
**If NO-GO**: Address issues and re-run `/prompts:kiro-spec-change $1 "updated description"`
