...
- If `-y` flag provided: Auto-approve change design
...
- If sequential is **false**: Read `{{KIRO_DIR}}/settings/rules/tasks-parallel-analysis.md` for parallel criteria
...
- Apply `(P)` markers for parallelizable tasks (omit in sequential mode)
...
- **Suggested Action**: "Run `/prompts:kiro-spec-change-tasks $1 -y` to auto-approve and proceed"

**Missing Change Documents**:
- **Stop Execution**: change-request.md and change-design.md must exist
- **Suggested Action**: "Run `/prompts:kiro-spec-change $1` and `/prompts:kiro-spec-change-design $1` first"
...
- **IMPORTANT**: Clear conversation history and free up context before running `/prompts:kiro-spec-change-impl`

**If Change Tasks Approved**:
- Execute specific task: `/prompts:kiro-spec-change-impl $1 C1.1`
- Execute multiple: `/prompts:kiro-spec-change-impl $1 C1,C2`
- Execute all: `/prompts:kiro-spec-change-impl $1`

**If Modifications Needed**:
- Provide feedback and re-run `/prompts:kiro-spec-change-tasks $1`
